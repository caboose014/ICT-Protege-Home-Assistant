...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    client.set_configuration(
        doors=door_ids, areas=area_ids, inputs=input_ids, outputs=output_ids
    )
    ...
    for d in door_ids:
        valid_device_identifiers.add((DOMAIN, f"door_{d}"))
    for a in area_ids:
        valid_device_identifiers.add((DOMAIN, f"area_{a}"))
    for i in input_ids:
        valid_device_identifiers.add((DOMAIN, f"input_{i}"))
    for o in output_ids:
        valid_device_identifiers.add((DOMAIN, f"output_{o}"))
